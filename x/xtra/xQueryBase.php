 ...
 * @todo
 */
	class xQueryBase extends Xengine {
		var $BACKUP_DIR;
...
/*		function autoRun(){
			...
			$back_dir = XPHP_DIR.'/xQueryBase/';
			$old_dir = scandir($back_dir);
			if(!empty($old_dir )){
				foreach($old_dir as $k => $f){
					if(!is_dir(XPHP_DIR.'/xQueryBase/'.$f)){
						$ext = explode('.',$f);
						if( strtolower($ext[count($ext)-1]) == 'sql' ){
							unlink(XPHP_DIR.'/xQueryBase/'.$f);
						}
					}
				}
			}
...
*/
		function __construct(){
			$this->BACKUP_DIR = XPHP_DIR.'/xQueryBase/'.WWWLESS_HOST;
	    ...
	    public function backup($backup=false,$relocate=true){
	    	...
	    	$file = $this->BACKUP_FILE;
	   		...
	   		if($backup && $relocate){
				$q->Backup($file);
				header("Location: ".str_replace($_SERVER['DOCUMENT_ROOT'],'',APP_DIR).'/queryBase/backup');
			}
