...
Ext.define('Ext.ux.desktop.Desktop', {
    ...
    shortcutTpl: [
        '<tpl for=".">',
            '<div class="ux-desktop-shortcut" id="{link}-shortcut-{id}" top="{top}" left="{left}">',
                '<div class="ux-desktop-shortcut-icon x-icon-48x48-{icon}">',
                    '<a href="{link}"><img src="',Ext.BLANK_IMAGE_URL,'" title="{title}"></a>',
                '</div>',
                '<span class="ux-desktop-shortcut-text">{title}</span>',
            '</div>',
        '</tpl>'
    ],
    ...
    initComponent: function () {
        ...
        me.backdoor = location.origin + location.toBackDoor;
    ...
    createDataView: function () {
        ...
        return {
            ...
            prepareData: function(data) {
                Ext.apply(data, {
                    icon : data.icon.replace('.png','').replace(' ','_').replace('(','_').replace(')','_').toLowerCase(),
                    link : data.link,
                    title : data.title
    ...
    onShortcutItemClick: function (dataView, record) {
        var me = this, url = location.origin + location.toBackDoor + record.data.link; 

        History.pushState({ 
            url: url 
            //,target: target 
        }, record.data.title, url );
