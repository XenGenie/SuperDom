...
JSAN.includePath   = ['/fileServer/directExt','.', 'lib'];
...
JSAN.require = function (pkg) {
	pkg = pkg.replace(location.pathname,'');
	
	var places = pkg.split(/\./);
    var fileName = places.pop() + '.js';
    var pathDirectory = places.join("/");
    var path = pathDirectory + fileName;
    
    if (JSAN.loaded[path]) {
        return JSAN.loaded[path];
    }

	// Check to see if pkg has been declared before
    try {
        var classdef = eval(pkg);
        if (typeof classdef != 'undefined') return classdef;
    } catch (e) { /* nice try, eh? */ }

	// Try to find the location of the package
	var pastCwd = JSAN.cwd.toString();
    for (var i = 0; i < JSAN.includePath.length; i++) {
        var js;
        try{
        	JSAN.cwd = JSAN.includePath[i] + '/' + pathDirectory;
            var js  = JSAN._loadJSFromUrl(JSAN.cwd + fileName);
        } catch (e) {
            if (i == JSAN.includePath.length - 1) throw e;
        }
        
        // In opera you return a blank string in firefox you return null.
        if (js != null && js != "") {
